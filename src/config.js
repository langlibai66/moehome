/**
 * MoeWah Homepage Configuration
 * 所有可配置内容集中管理，便于维护和更新
 */

window.HOMEPAGE_CONFIG = {
  // ========== 站点基础配置 ==========
  site: {
    name: "杨晨旭",
    tagline: "开发者 / 技术爱好者",
    url: "https://yourblog.com/rss.xml",
    ogImage: "/images/avatar.webp",
  },

  // ========== SEO 配置 ==========
  seo: {
    title: "🐱 user@host:~|",
    description: "欢迎访问杨晨旭的个人主页，这里聚合了我的所有在线身份与项目。",
    keywords: ["杨晨旭", "个人主页", "开发者"],
    og: {
      title: "杨晨旭 - 个人主页",
      description: "开发者 / 技术爱好者",
      image: "",
    },
  },

  // ========== 独立页面 SEO ==========
  pages: {
    404: {
      title: "页面未找到",
      description: "抱歉，您访问的页面不存在或已被删除",
      robots: "noindex, nofollow",
    },
    moments: {
      title: "动态",
      icon: "fa-solid fa-bolt",
      tagline: "我的碎片化分享，记录生活点滴与瞬间感悟。",
      description: "个人动态，记录生活点滴",
      keywords: ["动态", "瞬间"],
    },
    guestbook: {
      title: "留言",
      icon: "fa-solid fa-comments",
      tagline: "欢迎在这里留下你的足迹。",
      description: "留言板",
      keywords: ["留言板"],
    },
  },

  // ========== 主题配色 ==========
  theme: {
    default: "auto",
    defaultScheme: {
      light: null,
      dark: "catppuccinMocha",
    },
  },

  // ========== 导航栏 ==========
  nav: {
    enabled: false,
    brand: {
      showPrompt: true,
      hoverText: "~/whoami",
    },
    menus: [],
  },

  // ========== 个人信息 ==========
  profile: {
    name: "杨晨旭",
    tagline: {
      prefix: "🐱",
      highlight: "欢迎来到我的主页",
    },
    avatar: "images/avatar.webp",
  },

  // ========== Favicon ==========
  favicon: {
    path: "",
  },

  // ========== 身份标签 ==========
  identity: ["Hi, I'm 杨晨旭.", "开发者", "技术爱好者"],

  // ========== 兴趣领域 ==========
  interests: ["软件开发", "技术分享", "开源项目"],

  // ========== 装备 ==========
  gear: [],

  // ========== 终端配置 ==========
  terminal: {
    title: "🐱 user@host:~|",
    prompts: [
      { command: "whoami", output: "identity" },
      { command: "cat interests.txt", output: "interests" },
      { command: "./wisdom.sh", output: "dynamic" },
    ],
  },

  // ========== 名人语录 ==========
  quotes: [
    "Stay hungry, stay foolish.",
    "Talk is cheap. Show me the code.",
    "Simplicity is the ultimate sophistication.",
    "The best way to predict the future is to invent it.",
  ],

  // ========== 音乐 ==========
  music: {
    enabled: false,
    volume: 0.5,
    autoplay: false,
    playMode: "list",
    mode: "meting",
    meting: { server: "netease", type: "playlist", id: "", apis: [] },
    local: [],
  },

  // ========== 动画 ==========
  animation: {
    fadeInDelay: 1000,
    typingSpeed: 60,
    quoteDisplayTime: 4000,
    quoteDeleteSpeed: 42,
  },

  // ========== RSS ==========
  rss: {
    enabled: false,
    url: "https://yourblog.com/rss.xml",
    count: 6,
    openInNewTab: true,
    title: { text: "", icon: "fa-solid fa-newspaper" },
    display: { showDate: true, showDescription: true, maxDescriptionLength: 100 },
  },

  // ========== GitHub 项目 ==========
  projects: {
    enabled: true,
    title: { text: "我的项目", icon: "fa-solid fa-folder-open" },
    githubUser: "https://github.com/yourusername",
    count: 6,
    exclude: [".github"],
  },

  // ========== GitHub 贡献图 ==========
  contribution: {
    enabled: true,
    useRealData: false,
    githubUser: "",
  },

  // ========== 个人动态 ==========
  moments: { enabled: false, memosUrl: "", count: 10, tags: [], showSkeleton: true },

  // ========== 留言板 ==========
  guestbook: {
    enabled: false,
    provider: "waline",
    server: "",
    site: "MoeHome",
    placeholder: "欢迎留下你的信号...",
    limits: { comments: { newest: 30, hot: 30 }, barrage: { pinned: 5, hot: 10, latest: 5 } },
  },

  // ========== 链接模块 ==========
  linksConfig: {
    enabled: true,
    title: { text: "链接导航", icon: "fa-solid fa-link" },
  },

  // ========== 社交链接 ==========
  links: [
    {
      name: "简历",
      description: "我的在线简历",
      url: "https://your-resume-link.com",
      icon: "fa-solid fa-file-lines",
      brand: "resume",
      external: true,
      color: "#6366f1",
      enabled: true,
    },
    {
      name: "GitHub",
      description: "开源项目 & 代码",
      url: "https://github.com/yourusername",
      icon: "fa-brands fa-github",
      brand: "github",
      external: true,
      color: "#58a6ff",
      enabled: true,
    },
    {
      name: "Email",
      description: "联系我",
      url: "mailto:your-email@example.com",
      icon: "fa-solid fa-envelope",
      brand: "email",
      external: false,
      color: "#ea4335",
      antiCrawler: true,
      enabled: true,
    },
    {
      name: "小红书",
      description: "我的小红书主页",
      url: "https://www.xiaohongshu.com/user/profile/your-id",
      icon: "fa-solid fa-book-open",
      brand: "xiaohongshu",
      external: true,
      color: "#fe2c55",
      enabled: true,
    },
    {
      name: "博客",
      description: "我的个人博客",
      url: "https://yourblog.com",
      icon: "fa-solid fa-pen-nib",
      brand: "blog",
      external: true,
      color: "#00ff9f",
      enabled: true,
    },
  ],

  // ========== 赞赏 ==========
  donation: { enabled: false, title: { text: "赞助支持", icon: "fa-solid fa-mug-hot" }, message: "", methods: [] },

  // ========== 页脚 ==========
  footer: {
    copyright: { year: "2025", name: "杨晨旭", url: "" },
    icp: { enabled: false, number: "", url: "https://beian.miit.gov.cn/" },
  },

  // ========== 安全提示 ==========
  notice: { enabled: false, type: "warning", icon: "fa-solid fa-shield-halved", text: "" },

  // ========== 统计 ==========
  analytics: {
    googleAnalytics: { enabled: false, id: "" },
    microsoftClarity: { enabled: false, id: "" },
    umami: "",
    customScripts: [],
  },
};

function formatIdentity() { return window.HOMEPAGE_CONFIG.identity.join(" / "); }
function formatInterests() { return window.HOMEPAGE_CONFIG.interests.join(" / "); }
